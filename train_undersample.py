...
max_epochs = 10
learning_rate = 3e-5
MAX_CHUNKS = 100  # Define maximum chunk number
...
def text_preprocessing(text, mbti_type):
    ...
    # Detect and replace specified MBTI type
...
# Load data
data = pd.read_csv("./pandora/pandora_comments/filtered_processed_comments.csv")
...
llama_embeddings = torch.load("./features_tensor/pan_features_tensor_5.pt")
...
def undersample_with_pairing(sentences, labels, embeddings):
    label_counts = Counter([label_to_mbti(torch.tensor(label)) for label in labels])
    min_count = min(label_counts.values())
    
    new_sentences, new_labels, new_embeddings = [], [], []

    for mbti_type in MBTIs:
        mbti_samples = [(s, l, e) for s, l, e in zip(sentences, labels, embeddings) if label_to_mbti(torch.tensor(l)) == mbti_type]
        if len(mbti_samples) > min_count:
            mbti_samples = random.sample(mbti_samples, min_count)
        new_sentences.extend([s for s, l, e in mbti_samples])
        new_labels.extend([l for s, l, e in mbti_samples])
        new_embeddings.extend([e for s, l, e in mbti_samples])
    
    opposites = {'I': 'E', 'E': 'I', 'N': 'S', 'S': 'N', 'T': 'F', 'F': 'T', 'J': 'P', 'P': 'J'}

    paired_sentences, paired_labels, paired_embeddings = [], [], []
    for mbti_type in MBTIs:
        opposite_mbti = ''.join([opposites[c] for c in mbti_type])
        mbti_samples = [(s, l, e) for s, l, e in zip(new_sentences, new_labels, new_embeddings) if label_to_mbti(torch.tensor(l)) == mbti_type]
        opposite_samples = [(s, l, e) for s, l, e in zip(new_sentences, new_labels, new_embeddings) if label_to_mbti(torch.tensor(l)) == opposite_mbti]
        
        pair_count = min(len(mbti_samples), len(opposite_samples))
        
        paired_sentences.extend([s for s, l, e in mbti_samples[:pair_count]])
        paired_labels.extend([l for s, l, e in mbti_samples[:pair_count]])
        paired_embeddings.extend([e for s, l, e in mbti_samples[:pair_count]])
        
        paired_sentences.extend([s for s, l, e in opposite_samples[:pair_count]])
        paired_labels.extend([l for s, l, e in opposite_samples[:pair_count]])
        paired_embeddings.extend([e for s, l, e in opposite_samples[:pair_count]])
    
    return paired_sentences, paired_labels, paired_embeddings

train_sentences, y_train, train_llama_embeds = undersample_with_pairing(train_sentences, y_train, train_llama_embeds)
val_sentences, y_val, val_llama_embeds = undersample_with_pairing(val_sentences, y_val, val_llama_embeds)
...
with open('./MBTI_label_embedding_pandora.json', 'r') as f:
